...
import { dispatchWebhooks } from "../_shared/webhookDispatch.ts";
...
async function handleOrder(admin: any, event: string, reference: string, total_amount: number, currency: string) {
  ...
  if (event === "payment.failed") {
    ...
    const { data: failedItems } = await admin.from("order_items").select("seller_id").eq("order_id", order.id);
    const sellerIds = Array.from(new Set((failedItems || []).map((i: any) => i.seller_id)));
    for (const sellerId of sellerIds) {
      await dispatchWebhooks(admin, sellerId, "order.failed", { order_ref: order.order_ref, buyer_email: order.buyer_email });
    }
    return;
  }
  ...
  // Notify each seller's registered webhooks that their sale is confirmed.
  const sellerIds = Array.from(new Set(items.map((i: any) => i.seller_id)));
  for (const sellerId of sellerIds) {
    const sellerItems = items.filter((i: any) => i.seller_id === sellerId);
    await dispatchWebhooks(admin, sellerId, "order.completed", {
      order_ref: order.order_ref,
      buyer_email: order.buyer_email,
      buyer_name: order.buyer_name,
      items: sellerItems.map((i: any) => ({ product_id: i.product_id, product_title: i.product_title, quantity: i.quantity, amount: i.seller_credit })),
    });
  }
  ...
  if (!order.buyer_id && order.buyer_email) {
    for (const item of items) {
      ...
      // Also register a PIN-based access row so the buyer can retrieve
      // their purchase from any device using order.buyer_pin, without
      // needing to remember which email they checked out with.
      await admin.from("buyer_access").insert({
        email: order.buyer_email, pin: order.buyer_pin, order_id: order.id,
