...
export function SellerBroadcasts() {
  ...
  async function send() {
    ...
    let buyerIds: string[] = [];
    if (target === "all") {
      const res = await sb.from("order_items")
      ...
      buyerIds = Array.from(new Set(
        rows
          .map((row) => row.order)
          .filter((o: any) => o && o.status === "confirmed" && o.buyer_id)
          .map((o: any) => o.buyer_id as string),
      ));
    } else {
      ...
      const res = await sb.from("order_items")
        ...
        .eq("seller_id", user.id);
      const rows: any[] = res.data || [];
      const match = rows.find((row) => row.order && row.order.buyer_email === searchEmail.trim().toLowerCase() && row.order.buyer_id);
      buyerIds = match ? [match.order.buyer_id as string] : [];
    }
